    ...
    public class Plugin : IEmbyBeautifyPlugin, IServerEntryPoint
    {
        ...
        private async Task InitializeServicesAsync()
        {
            try
            {
                ...
                // Create logger factory for service dependencies
                var loggerFactory = new Microsoft.Extensions.Logging.LoggerFactory();
                loggerFactory.AddProvider(new EmbyLoggerProvider(_logManager));

                // Initialize ConfigurationManager
                var configLogger = loggerFactory.CreateLogger<Services.ConfigurationManager>();
                _configurationManager = new Services.ConfigurationManager(configLogger);
                _logger.Debug("ConfigurationManager initialized");

                // Initialize ThemeManager with ConfigurationManager dependency
                var themeLogger = loggerFactory.CreateLogger<Services.ThemeManager>();
                _themeManager = new Services.ThemeManager(themeLogger, _configurationManager);
                _logger.Debug("ThemeManager initialized");

                // Initialize supporting services for StyleInjector
                var errorHandlingLogger = loggerFactory.CreateLogger<Services.ErrorHandlingService>();
                var errorHandlingService = new Services.ErrorHandlingService(errorHandlingLogger);
                
                var cacheLogger = loggerFactory.CreateLogger<Services.StyleCacheService>();
                var styleCacheService = new Services.StyleCacheService(cacheLogger, _configurationManager);

                // Initialize StyleInjector with all dependencies
                var styleLogger = loggerFactory.CreateLogger<Services.StyleInjector>();
                _styleInjector = new Services.StyleInjector(
                    styleLogger,
                    _themeManager,
                    _configurationManager,
                    errorHandlingService,
                    styleCacheService);
                _logger.Debug("StyleInjector initialized");

                // Initialize ThemeManager after all dependencies are ready (async to improve startup time)
                _ = Task.Run(async () => 
                {
                    try
                    {
                        await _themeManager.InitializeAsync();
                        _logger.Debug("ThemeManager initialization completed");
                    }
                    catch (Exception ex)
                    {
                        _logger.ErrorException("ThemeManager initialization failed", ex);
                    }
                });
