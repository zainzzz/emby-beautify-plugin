...
namespace EmbyBeautifyPlugin.Services
{
    ...
    public class StyleCacheService
    {
        ...
        /// 清理文件缓存
        /// </summary>
        private Task CleanupFileCacheAsync()
        {
            try
            {
                if (!Directory.Exists(_cacheDirectory))
        ...
        /// 修剪内存缓存
        /// </summary>
        private Task TrimMemoryCacheAsync()
        {
        ...
        /// 检查文件缓存大小
        /// </summary>
        private Task CheckFileCacheSizeAsync()
        {
            try
            {
                if (!Directory.Exists(_cacheDirectory))
        ...
        /// 获取文件缓存信息
        /// </summary>
        private Task<(int Count, long Size)> GetFileCacheInfoAsync()
        {
            try
            {
                if (!Directory.Exists(_cacheDirectory))
                ...
                return Task.FromResult((files.Length, totalSize));
            }
            catch (Exception ex)
            {
